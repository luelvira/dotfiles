...
- [[#editing-configuration][Editing configuration]]
  - [[#tabs-widths][Tabs Widths]]
  - [[#commenting-lines][Commenting lines]]
  - [[#autimatically-clean-whitespace][Autimatically clean whitespace]]
...
(use-package evil
 ...
 :config
 (add-hook 'evil-mode-hook 'lem/evil-hook)
 (evil-mode 1)
...
The following setting was moved to the evil declaration
...
Maximize window by default and make it transparent
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ;; Set the font
  (setq lem/defaultFont "Fira Code"
        lem/fixed-pitch "Fira Code"
        lem/variable-pitch "Noto Sans")

  (set-face-attribute 'default nil :font lem/defaultFont :height 120)
  (set-face-attribute 'fixed-pitch nil :family lem/fixed-pitch :height 1.0 :inherit 'default)
  (set-face-attribute 'variable-pitch nil :family lem/variable-pitch :weight 'regular :inherit 'default)
...
** Autimatically clean whitespace
#+begin_src emacs-lisp
...
(use-package vertico
  ...
  :bind (:map vertico-map
         ...
         ("C-f" . vertico-exit)
         :map minibuffer-local-map
         ("M-h" . lem/minibuffer-backward-kill))
  :custom
  (vertico-cycle t)
  :custom-face
...
:defer t
  :hook (org-mode . lem/org-mode-visual-fill))
#+end_src
** Configure flyspell
...
(dolist (face '((org-level-1 . 1.2)
                ...
                (org-level-7 . 1.1)
                (org-level-8 . 1.1)))
(set-face-attribute (car face) nil :font lem/variable-pitch :weight 'medium :height (cdr face)))
...
The configuration file of this section was moved to [[file:~/Documents/git/dotfiles/emacs.d/org-workflow.org][org-workflow.org]].
