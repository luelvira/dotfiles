...
- [[#dashboard][DASHBOARD]]
  ...
  - [[#startup-performance-1][Startup performance]]
...
(set-face-attribute 'variable-pitch nil :font "Noto Sans"  :weight 'regular)
...
I have a script which try to keep sync with a repository on [[https://codeberg.org/luelvira/Org][codeberg]]. This repo contains the org files only, and it is named sync

#+begin_src emacs-lisp
  (defun lem/sync ()
  "Sync the Org foler with an external script"
  (when (and (eq major-mode 'org-mode)
             (s-prefix? (expand-file-name org-directory)
                        (buffer-file-name (current-buffer))))
    (shell-command-to-string "/home/lucas/.local/bin/sync.sh")))

  (add-hook 'after-save #'lem/sync)
#+end_src
